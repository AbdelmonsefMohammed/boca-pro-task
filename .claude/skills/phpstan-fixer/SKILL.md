...
license: MIT
compatibility: Requires PHPStan installed in project. Written against PHPStan 2.x + Larastan 3.x (this project: phpstan/phpstan 2.2.8, larastan/larastan v3.10.0, level 5).
metadata:
  version: "2.0.0"
---
...
## PHPStan 2.x / Larastan 3.x Specifics

These are current-version behaviors that change how you should react to output, not just cosmetics:

### Some identifiers cannot be ignored at all

PHPStan 2.x marks certain error identifiers `(non-ignorable)` right in the CLI output —
`ignoreErrors` / baseline entries targeting them are rejected. Seen in practice:
`property.visibility`, `method.childParameterType`, `method.childReturnType`,
`parameter.notOptional`, `ignore.unmatchedLine`, `ignore.unmatched`. If an error is
non-ignorable, suppressing it isn't on the table — the code has to change. These are
frequently a symptom of a subclass accidentally overriding a parent property/method with an
incompatible signature (same name, different type) — check for that specifically, since the
fix is usually to rename the child member, not to loosen types.

### Baseline entries go stale and become blocking errors

`phpstan-baseline.neon` entries are pinned to an exact message + path + count. When the
underlying code changes (or someone else's change makes the original error stop
occurring), the stale entry itself fails the run with `ignore.unmatchedLine` /
`ignore.unmatched` (`reportUnmatchedIgnoredErrors` defaults to `true`). Fix: open the
referenced file, confirm the original error genuinely no longer reproduces, then delete
that one entry from the baseline — don't regenerate the whole baseline file blindly, since
that can silently re-swallow unrelated new errors.

### Larastan boots the real Laravel app — always run inside Docker

Larastan boots your actual application (service providers, facades, config) to resolve
Eloquent/Facade types, so `phpstan analyse` needs a working app context — correct PHP
version, extensions, and `.env`. A bootstrap crash (e.g. `Class "Redis" not found`) almost
always means you're running PHPStan on the wrong PHP binary or outside the app's container,
not a real static-analysis bug.

**Always run `phpstan analyse` (and any fix-verification) inside Docker, never on the host:**
`docker exec -w /var/www/html atlas-octane-app vendor/bin/phpstan analyse`. The host PHP
version/extensions don't match production and will produce false failures. No exceptions.
